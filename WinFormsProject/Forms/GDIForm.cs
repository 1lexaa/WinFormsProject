...
Набор универсальных методов для отображения графики (рисования)
Динамическое обновление:
- не рекомендуется использовать CreateGraphics, лучше вызывать перерисовку элементы
   (формы)
- в событии перерисовки (Paint) также не вызываем CreateGraphics, а используем
   переданную в событии ссылку e.Graphics
- устанавливаем свойство DoubleBuffered элемента (формы) в true, это создает
   плавность перерисовки (вместо отображения каждой команды перерисовки,
   все команды буферизируются и отображаются разом)

Если нет необходимости в динамике, то от рекомендаций можно отклоняться
 */

/*
 Д.З. Перенести в класс Ball данные о "рисующих" кисти и пере.
    Обеспечить проверку выхода шарика за пределы формы: нижняя и правая границы
*/
